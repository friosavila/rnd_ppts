...
- This approach its so simple that canonical DiD, a 2x2 design, simply compares means of the outcome variable (before after x treated non-treated) to identify treatment effects. 
  - Thus it can be used even if outcome is a limited dependent variable (binary, count, etc) 

## Canonical DiD: 2x2 design

- Consider the case where we have panel data (easiest scenario)
    - We have two groups, those in treatment (D=1) and control (D=0)
    - and all units are observed for two periods, a before and after (T=0,1)
- If we assume full heterogeneity, the potential and realized outcomes for each unit are:
$$\begin{aligned}
...
In this framework, TE for $i$ can be defined as:
...
First, Decompose it:
$$E_1(y_{i,1}(0))=E_1(y_{i,0}(0)) + E_1(\lambda_i)$$ 
...
Where $\theta$ is the ATT on the treated.
...
- Cannonical DiD is stright forward, but its also very restrictive.
    - What if we can identify and follow individuals over time? (Individual fixed effects)
...
## And some times we making even harder

- Advanced questions in DID
...
- Availability of many individuals that can be followed across time can be easily adapted in the regression based DID approach.
...
- For multiple periods, the same logic applies. Instead of controling for a before/after $t$, we can control for a time fixed effect.

$$y_{it} = \theta (W_{it}) + \beta_i + \gamma_t + \epsilon_{it}
$$

- $W_{it} =1$ only for treated units in the post-treatment period, and $W_{it} =0$ otherwise.
...
- Canonical DID assumes PTA holds, implicitly assuming TE is homogenous across all groups, or Treated and control groups are similar.
- But what if that isn't the case?
    - We may want to control for unit characteristics $X$
...
- This is a simple extension of the DID regression, 
    ...
    - If that isn't the case, results are biased.

## Conditional Parallel Trends Assumption (CPTA)
...
- Most approaches usually provide this aggregate ATT, or do separate estimates for each subgroup.
    
## How is it done Empirically? (2x2 case)

- Sant'Anna and Zhao (2020) propose few solutions:
    - Outcome Regression (Separate models for treated and control/pre and post periods)
    - Inverse Probability Weighting (IPW)
    - Combination of both

- Wooldridge(2021) implicity outcome regression with interactions

$$\begin{aligned}
y_{it} &= \alpha     &&+ \beta D            &&+ \gamma t &&+ \theta (D \times t)  \\
         &+ \lambda X  &&+ \lambda_D X \times D  &&+ \lambda_D X \times t  &&+ \lambda_{DT} \tilde X (D \times t)   \\\  
         &+ \epsilon_{it}
\end{aligned}
$$
...
- Tough question!
    1. In these models, more controls means more interactions, thus more parameters to be estimated, and more unstable estimates. Unless you have large RC, you probably want to use only few controls.
...
- Standard OLS estimates identifies $\theta$ by comparing Pre and Post outcomes using various 2x2 designs.
    - contrasting units that changed treatment status (treated) 
    - with those that did not (controls, not-yet treated and **already treated**). 
...
- There are many solutions to the problem
    1. Avoid the problem using only pretreatment data for estimation of counterfactuals (`did2s` and `did_imputation`)
    2. Use only good controls when estimating the ATT (Callaway and Sant'Anna, 2021) (`csdid[2]`)
    3. Modify model especfication to achieve 1 or 2. (Wooldridge 2021, 2022, 2024; `jwdid`)
...
- Allows ATT ($\theta_{gt}$) to vary by group and time, using all not treated units as controls.
...
$$y_{it} = \beta_i + \lambda_t + \sum_{g=g_0}^G \sum_{t=0}^{t=g-2} \theta_{gt} \mathbb{1}(g, t) + \sum_{g=g_0}^G \sum_{t=g}^{t=T} \theta_{gt} \mathbb{1}(g, t) + \epsilon_{it}$$
...
- Allowing for $X$ heterogeneity is simple. Simply consider a flexible model with interactions!
$$\begin{aligned}
y_{it} &= \color{red}{\beta_0}  &&+ \beta_i &&+ \lambda_t &&+ \sum_{g=g_0}^G \sum_{t=g}^{t=T} \theta_{gt} \mathbb{1}(g, t) &&+ \\
    &\ \ \ \   \delta X_{i,t}  
    &&+ \sum_g \delta_g X_{i,t}  \mathbb{1}(g) 
    &&+ \sum_t \delta_t X_{i,t}  \mathbb{1}(t) 
...
```
- Both approaches will produce the same results once aggregations are 
done.
...
- As with OLS, one Big advantage of using OLS is that it can be used for any type of outcome.
...
- Solution (Wooldridge(2023)) Focus not on the observed outcome, but on the latent variable!

## Limited Dependent Variables 

There are **two** main changes to consider:

1. We change the model and focus on the latent variable (think GLM)
...
2. Most non-linear models have issues adding many fixed effects (incidental parameters problem)
...
- `jwdid` can estimate these type of models simply calling for `method()`.
    - Almost any model available in Stata can be called. Few cases may require tweaks (mostly two-word commands and clustered SE call)
...
```stata
estat [simple|group|calendar|event] /// Default uses "method()" options
...
- `jwdid` already incorporates this by using flexible models, but produces Average ATT's.
...
- Another common scenario is when treatment is different across groups.
    - Case 1: Treatment is the same, but dose response (0-100) varies
    - Case 2: Each treatment is different ( Treatment A, B or C)

- The `jwdid` framework can also be adapted to these scenarios, albeit with limitations.

## Case 1: Treatment Intensity
    
- The Treatment intensity should be defined between 0-1. 
- Impose the assumption that ATT(G,T)(X) is linear on the treatment intensity.
- This requires you to have a variable `trtvar` that defines treatment intensity.
...
- I wrote `jwdid` to be as flexible as possible, inspired by the work of Wooldridge (2021, 2022, 2023, 2024) 
    - some of the extensions and options go beyond Wooldridge's work (or were developed independently)
